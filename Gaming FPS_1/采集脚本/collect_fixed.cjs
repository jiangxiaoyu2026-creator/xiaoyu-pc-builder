...
// 采集GPU数据
async function collectGpu(page, gpuName) {
  const selected = await searchAndSelect(page, 'GPU', gpuName);
  if (!selected) {
    return null;
  }
...
async function main() {
  ...
  await page.goto('https://howmanyfps.com/games/fortnite');
  ...
  const gpus = [
    ...
    'RTX 4080 Super', 'RX 6500 XT', 'RX 970', 'Radeon RX 6400',
    ...
    'Radeon RX 7600', 'Radeon RX 7600 XT', 'Radeon RX 7650 GRE',
    'Radeon RX 9060 XT', 'Radeon RX 9060 XT 16GB', 'Radeon RX 9070',
    'Radeon RX 9070 XT', 'Radeon RX 970'
  ];

  let content = `# Fortnite GPU 帧率数据 — HowManyFPS\n## 游戏: Fortnite\n## 采集时间: ${new Date().toISOString().split('T')[0]}\n\n`;
  content += `| GPU | 分辨率 | Real-Time Average | 1% Lows |\n|-----|--------|-------------------|--------|\n`;

  console.log('=== Fortnite GPU 修复版采集 ===\n');

  const issues = [];

  for (let i = 0; i < gpus.length; i++) {
    const gpu = gpus[i];
    console.log(`[${i+1}/${gpus.length}] ${gpu}...`);

    const data = await collectGpu(page, gpu);

    if (!data) {
      console.log('  ❌ 未找到');
      issues.push({ gpu, reason: 'not_found' });
      continue;
    }
    ...
    for (const r of data.results) {
      if (!r.valid) {
        issues.push({ gpu, res: r.resolution, reason: r.reason });
        console.log(`  ⚠️ ${r.resolution}: 无数据 (${r.reason})`);
        hasIssue = true;
      } else {
        const problems = validateData(r.avg, r.low);
        if (problems.length > 0) {
          issues.push({ gpu, res: r.resolution, reason: problems.join(', ') });
          console.log(`  ⚠️ ${r.resolution}: ${problems.join(', ')}`);
          hasIssue = true;
        }
      }
    }
  ...
  fs.writeFileSync(`${DATA_DIR}/howmanyfps_fortnite_gpu_data_fixed.md`, content);

  // 保存问题报告
  if (issues.length > 0) {
    fs.writeFileSync(`${DATA_DIR}/fortnite_gpu_issues_fixed.json`, JSON.stringify(issues, null, 2));
  }

  console.log(`\n=== 采集完成 ===`);
  console.log(`成功: ${gpus.length - issues.filter(i => i.reason === 'not_found').length}`);
  console.log(`失败: ${issues.length}`);
  if (issues.length > 0) {
    console.log(`详见: fortnite_gpu_issues_fixed.json`);
  }
