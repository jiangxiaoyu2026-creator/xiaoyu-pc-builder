...
const MISSING_GPUS = [
  ...
  'RTX 3060 8 GB', 'RTX 4070 Super', 'RTX 4070 Ti Super', 'RTX 4080 Super'
];
...
async function main() {
  ...
  let content = '# Cyberpunk 2077 GPU 帧率数据 — HowManyFPS\n## 游戏: Cyberpunk 2077\n## 采集时间: ' + new Date().toISOString().split('T')[0] + '\n\n';
  content += '| GPU | 分辨率 | Real-Time Average | 1% Lows |\n|-----|--------|-------------------|--------|\n';

  console.log('=== Cyberpunk 2077 GPU 采集 ===\n');

  const issues = [];
  let successCount = 0;

  for (let i = 0; i < MISSING_GPUS.length; i++) {
    const hw = MISSING_GPUS[i];
    console.log('[' + (i+1) + '/' + MISSING_GPUS.length + '] ' + hw + '...');

    const data = await collectData(page, hw);
    ...
    // 每10个保存一次
    if ((i + 1) % 10 === 0) {
      console.log('\n--- 验证点: ' + (i+1) + '/' + MISSING_GPUS.length + ' ---');
      fs.writeFileSync(DATA_DIR + '/cyberpunk2077_gpu_temp.md', content);
    }
  }

  fs.writeFileSync(DATA_DIR + '/howmanyfps_cyberpunk2077_GPU_data_new.md', content);

  if (issues.length > 0) {
    fs.writeFileSync(DATA_DIR + '/cyberpunk2077_GPU_issues_new.json', JSON.stringify(issues, null, 2));
  }

  console.log('\n=== 采集完成 ===');
  console.log('成功: ' + successCount + '/' + MISSING_GPUS.length);
