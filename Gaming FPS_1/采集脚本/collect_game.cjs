...
// 侠盗猎车手 5 缺失的 CPU 列表（74款）
const MISSING_CPUS_GTAV = [
...
// 侠盗猎车手 5 缺失的 GPU 列表（27款）
const MISSING_GPUS_GTAV = [
...
async function searchAndSelect(page, type, searchTerm) {
  await page.click('[aria-label="Search for a ' + type + '"]', {force: true});
...
async function main() {
  ...
  const gameSlug = process.argv[3] || 'grand-theft-auto-v';
  const gameName = process.argv[4] || '侠盗猎车手 5';

  const typeLower = type.toLowerCase();
  const hardwareList = typeLower === 'gpu' ? MISSING_GPUS_GTAV : MISSING_CPUS_GTAV;
  const typeLabel = type.toUpperCase();
  const filePrefix = gameSlug.replace(/-/g, '');

  const browser = await chromium.connectOverCDP('http://localhost:9222', {timeout: 60000});
  const page = await browser.newPage();
  await page.setViewportSize({ width: 1920, height: 1080 });
  await page.goto('https://howmanyfps.com/games/' + gameSlug, {timeout: 60000, waitUntil: 'domcontentloaded'});
  await page.waitForTimeout(3000);

  // 检查页面是否有效
  const title = await page.title();
  if (title.includes('Not Found')) {
    console.log('错误: 游戏页面未找到 - ' + gameSlug);
    await browser.close();
    return;
  }
  ...
  for (let i = 0; i < hardwareList.length; i++) {
    ...
    if ((i + 1) % 10 === 0) {
      ...
      fs.writeFileSync(DATA_DIR + '/' + filePrefix + '_' + type + '_temp.md', content);
    }
  }

  fs.writeFileSync(DATA_DIR + '/howmanyfps_' + filePrefix + '_' + type + '_data_new.md', content);

  if (issues.length > 0) {
    fs.writeFileSync(DATA_DIR + '/' + filePrefix + '_' + type + '_issues.json', JSON.stringify(issues, null, 2));
  }
