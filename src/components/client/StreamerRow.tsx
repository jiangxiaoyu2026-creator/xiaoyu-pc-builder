        ...
        <div className={`grid ${isLiveMode ? 'grid-cols-[72px_minmax(0,1fr)_44px_76px] h-full px-3 py-0' : 'grid-cols-[68px_minmax(0,1fr)_56px_64px_18px] px-3 py-2'} gap-2 items-center group transition-colors relative ${showSuggestions ? 'z-[100]' : ''} ${isLiveMode ? 'bg-transparent hover:bg-white/[0.04] transition-colors' : (entry.item ? `${theme.bgLight} dark:bg-opacity-20` : 'hover:bg-slate-50 dark:hover:bg-slate-800/50')}`}>
            ...
            <div className="relative">
                ...
                {showSuggestions && query.trim().length > 0 && (
                    <div ref={suggestionsRef} className={`absolute ${index >= 6 ? 'bottom-full mb-2' : 'top-full mt-2'} left-0 right-0 ${isLiveMode ? `${isPixelLiveStyle ? `${liveStyleConfig.panelBg} ${liveStyleConfig.stampBorder} shadow-[5px_5px_0_#050505]` : isSoftLightLiveStyle ? `${liveStyleConfig.panelBg} ${liveStyleConfig.stampBorder} shadow-slate-900/15` : 'bg-gray-900 border-gray-700 shadow-black/50'}` : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700'} shadow-2xl ${isPixelLiveStyle && isLiveMode ? 'rounded-none border-2' : 'rounded-xl border'} z-[999] overflow-hidden max-h-[300px] overflow-y-auto`}>
                        ...
                        {!isLoading && suggestions.map((item, idx) => (
                            <div key={item.id} className={`px-4 ${isLiveMode ? 'py-2.5 text-[15px]' : 'py-2 text-sm'} flex justify-between cursor-pointer ${idx === highlightIndex ? (isLiveMode ? (isPixelLiveStyle ? `${liveStyleConfig.glowBg} text-gray-950` : isSoftLightLiveStyle ? `${liveStyleConfig.glowBg} text-white` : 'bg-gray-800 text-white') : `${theme.bgLight} ${theme.primary} transition-colors`) : (isLiveMode ? (isPixelLiveStyle ? `${liveStyleConfig.modelText} hover:brightness-110` : isSoftLightLiveStyle ? `${liveStyleConfig.modelText} hover:bg-black/[0.04]` : 'text-gray-300 hover:bg-gray-800') : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50')}`} onMouseDown={() => selectItem(item)}>
